...
var api_method = {
	...
	h5Pay:"alipay.trade.wap.pay",//手机网站h5支付
	precreate:"alipay.trade.precreate",//统一收单线下交易预创建
	appPay:"alipay.trade.app.pay",//app支付接口
...
var payH5 = function(params){
	...
	if(params.biz_content.return_url){
		body.return_url=params.biz_content.return_url;
	}
	if(params.biz_content.notify_url){
		body.notify_url=params.biz_content.notify_url;
	}
	if(params.biz_content.app_auth_token){
		body.app_auth_token=params.biz_content.app_auth_token;
	}
...
exports.initClient = function (params){
	...
	var isFile = /.*\.pem$/;
	try{
		private_key = isFile.test(params.private_key)?fs.readFileSync(params.private_key):params.private_key;
	}catch (err){
		console.log(err);
		return;
	}
